...
function processReceiverUpdate(data) {
	...
	MessageRate = (data.messages - prev_messages)/(now - last);
...
function initialize() {
	...
	// with initialization
	get_receiver().done(function(data){
		...
		histInterval = data.refresh/1000;
...
function end_load_history() {
	...
	if (false && PositionHistoryBuffer.length > 0) {
		var now, last=0;
